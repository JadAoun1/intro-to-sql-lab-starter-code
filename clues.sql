...
SELECT L.language
FROM countries AS C
JOIN countrylanguages AS L
ON C.code = L.countrycode
WHERE C.name = 'Holy See (Vatican City State)' AND L.isofficial = TRUE;
...
AND C.code NOT IN (SELECT countrycode FROM countrylanguages WHERE language != 'Italian')
AND C.region = 'Southern Europe'
AND C.name != 'Holy See (Vatican City State)'; -- Exclude Vatican City itself
...
WHERE CO.name = 'San Marino' AND C.name != 'San Marino';
...
JOIN countries AS CO
ON C.countrycode = CO.code
WHERE C.name LIKE 'Serra%'
AND C.name NOT LIKE 'Serravalle'
